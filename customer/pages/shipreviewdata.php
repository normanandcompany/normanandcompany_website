...
function sendShipReviewJson(array $payload, int $status = 200): void
{
...
try {
    ...
    error_log('Ship review database connection failed: ' . $e->getMessage());
    sendShipReviewJson([
        'success' => false,
...
if (empty($_SESSION['ship_review_csrf'])) {
    $_SESSION['ship_review_csrf'] = bin2hex(random_bytes(32));
}

$shipIdValue = $_SERVER['REQUEST_METHOD'] === 'POST'
    ? ($_POST['ship_id'] ?? '')
    : ($_GET['ship_id'] ?? '');

if (!is_scalar($shipIdValue) || !ctype_digit((string) $shipIdValue) || (int) $shipIdValue < 1) {
    sendShipReviewJson([
        'success' => false,
        'message' => 'Select a valid ship.'
    ], 422);
}

$shipId = (int) $shipIdValue;
$userId = (int) getUserId();

try {
    $shipStatement = $pdo->prepare("
        SELECT id, ship_name
        FROM ships
        WHERE id = :ship_id
        ...
        LIMIT 1
    ");
    $shipStatement->execute([':ship_id' => $shipId]);
    $ship = $shipStatement->fetch(PDO::FETCH_ASSOC);

    if (!$ship) {
        sendShipReviewJson([
            'success' => false,
            'message' => 'That ship is not available for review.'
        ], 404);
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        sendShipReviewJson([
            ...
            'csrf_token' => $_SESSION['ship_review_csrf']
        ]);
    }
    ...
    if (!hash_equals((string) $_SESSION['ship_review_csrf'], $csrfToken)) {
        sendShipReviewJson([
            'success' => false,
    ...
    $saveStatement->bindValue(':ship_id', $shipId, PDO::PARAM_INT);
    ...
    error_log('Ship review query failed: ' . $e->getMessage());
    sendShipReviewJson([
        'success' => false,
        'message' => 'Your review could not be saved right now.'
    ], 500);
} catch (Throwable $e) {
    error_log('Ship review request failed: ' . $e->getMessage());
    sendShipReviewJson([
        'success' => false,
